...
describe("optimizeImage", () => {
  it("downscales images wider than the cap to the max width", async () => {
    const result = await optimizeImage(await makePng(1000, 400, true), { maxWidth: 800 });
    expect(result.width).toBe(800);
  });

  it("keeps images at or under the cap at their original width", async () => {
    const result = await optimizeImage(await makePng(400, 200, true), { maxWidth: 800 });
    expect(result.width).toBe(400);
  });

  it("reports the source format", async () => {
    const result = await optimizeImage(await makePng(120, 120, false));
    expect(result.format).toBe("png");
  });

  it("never returns more bytes than the input", async () => {
