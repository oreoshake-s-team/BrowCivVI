...
describe("geographyErrors", () => {
  it("flags Pella placed east of the Hellespont", () => {
    ...
    expect(geographyErrors(map).some((error) => error.includes("Pella"))).toBe(true);
  });

  it("flags a map with no river", () => {
    const map = createGameMap([], []);
    expect(geographyErrors(map).some((error) => error.includes("Granicus river"))).toBe(true);
  });

  it("flags Sparta placed north of Corinth", () => {
    const map = createGameMap([], [city("sparta", 1, 2), city("corinth", 1, 6)]);
    expect(geographyErrors(map).some((error) => error.includes("Sparta must lie"))).toBe(true);
  });
  ...
  it("flags an Aegean rendered as a thin sliver", () => {
    ...
    expect(geographyErrors(map).some((error) => error.includes("sliver"))).toBe(true);
  });
});
...
describe("anachronismErrors", () => {
  it("flags a city founded after the campaign", () => {
    ...
    expect(anachronismErrors(map).some((error) => error.includes("not yet founded"))).toBe(true);
  });

  it("flags a city with no attestation date", () => {
    const map = createGameMap([], [city("nowhere", 0, 0, { dated: false })]);
    expect(anachronismErrors(map).some((error) => error.includes("no attestation date"))).toBe(
      true,
    );
  });
});
...
describe("cityTerrainErrors", () => {
  it("flags a city placed on a water tile", () => {
    ...
    expect(cityTerrainErrors(map).some((error) => error.includes("non-land"))).toBe(true);
  });
