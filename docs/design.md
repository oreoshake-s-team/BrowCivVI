...
- Default real store: **Postgres + `JSONB`** — _not_ a `TEXT` dump. The match aggregate is stored as a binary, indexable `JSONB` column; the **schema is enforced in TypeScript** at the read/write boundary and versioned with the code.
- **Evolving model strategy:** every stored aggregate carries a `schemaVersion`, and a single **tolerant decoder** (`decode(raw) -> current`) runs on read. Because additive field changes are the overwhelming common case, the decoder **defaults any absent field** (deriving from siblings where needed) rather than threading a version-by-version upcaster chain — adding a field is a one-line default, no version bump. `schemaVersion` is kept purely as a **tripwire**: the decoder rejects a version _newer_ than the running engine understands (no silent downgrade) and leaves an explicit pre-pass hook for the rare _destructive_ transform (rename / re-type / fix wrong data), which is the only case that bumps the version. No SQL migration for state-shape changes. (This technique is identical for a document DB — so an evolving model is _not_ itself a reason to prefer one engine over the other.)
